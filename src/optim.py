...
def get_cl_init(N, loc=-6, scale=0.1):
    cl = np.random.normal(loc=loc, scale=0.1, size=N)
    #cl[0] = 0
    cl = torch.tensor(cl, requires_grad=True)
    return cl
...
def optimize_control(loss_fn, cl, g, lambd=0, verbose=False, return_hist=False, lr=0.1, num_steps=10000):
    ...
    for i in tqdm(range(num_steps), disable=not verbose):
        ...
        with torch.no_grad():
            ...
            if i % 10 == 0:
                hist["params"].append(params.numpy())
                hist["params_sm"].append(torch.sigmoid(params).numpy())
    with torch.no_grad():
        hist["final_params_sm"] = torch.sigmoid(params).numpy()
    ret = (params, loss)
    if return_hist:
        ret += (hist, )
    return ret
